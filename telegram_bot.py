...
async def get_active(message: types.Message):
    ...
    if user_id in USERS:
        ...
        await message.reply("Для начала зарегистрируйтесь! Используйте команду `/login <login> <password> <email>`",
...
async def login(message: types.Message):
    ...
    USERS[message.from_user.id] = Regulator(System(log, passw, email))
    print(USERS)
...
async def add_bot(message: types.Message):
    ...
    if user_id in USERS:
        try:
            id, price, time = message.text.split()[1:]
        except:
        ...
        regulator.newsession(id=id, price=price, timedelta=time)
...
async def cmd_start(message: types.Message):
    ...
    await message.answer("Добро пожаловать! Используйте команду `/login <login> <password> <email>` для регистрации",
                         parse_mode="MarkDownV2")
